...
工作目录 `_src/`(收件箱 + 原始图源)在仓库内但**整目录 git-ignored**——仓库只存最终 WebP 海报。
默认 `POSTERS_SRC=_src`(仓库根下),可用环境变量覆盖。
...
echo "2410.24164" >> _src/_inbox/queue.txt

# 2) 在 Claude 会话里触发 paper-notes skill（“处理 paper inbox / 生成 paper notes”）
#    → 它提取论文列表、派 Sonnet 子代理逐篇生成到 _src/<slug>/，再自动跑下面的发布脚本

# 3) 发布脚本（paper-notes 会自动调用；也可手动跑）：图转 WebP、改写 <img src>、
#    拷 meta.json 进 <slug>/（仓库根），并重建 index.html
pipeline/publish_to_site.sh
...
| 文件 | 作用 |
|---|---|
| `publish_to_site.sh` | 把 `_src/<slug>/` 的图转 WebP、改写 HTML 的 `<img src>`、拷 `meta.json` 进 `<slug>/`（仓库根），再调 `build_index.py` 重建 `index.html`。幂等。 |
