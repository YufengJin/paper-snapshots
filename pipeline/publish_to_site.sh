...
# Usage: pipeline/publish_to_site.sh
set -uo pipefail

HERE="$(cd "$(dirname "$0")" && pwd)"
SRC="${POSTERS_SRC:-$(cd "$HERE/.." && pwd)/_src}"   # in-repo working dir (_src; raw figs)
SITE="${SITE_DIR:-$(cd "$HERE/.." && pwd)}"                  # repo root (pipeline -> repo)
DST="$SITE"                                                  # posters live at the repo root
